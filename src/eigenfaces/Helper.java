...
public class Helper 
{
	...
	public static double[] readImagePixels(String imageFileName, int width, int height) 
			throws Exception 
	{
...
//		System.out.println(matrixSeqFileName);
			...
			double[][] eigenFaces) 
	{
		...
		if (diffImagePixels.length<pixelCount)
			pixelCount = diffImagePixels.length;
			...
			double[] meanImagePixels) throws Exception 
	{
		...
		if (meanImagePixels.length < pixelCount)
			pixelCount = meanImagePixels.length;
		for(int i = 0 ; i < eigenFaceCount ; i++) 
		{
			for(int j = 0 ; j < pixelCount ; j++) 
			{
		...
		double[] normalizedReconstructedPixels = new double[reconstructedPixels.length];
		for(int i = 0 ; i < reconstructedPixels.length ; i++) 
		{
	...
	public static double computeImageDistance(double[] pixelImage1, double[] pixelImage2) 
	{
		...
		if (pixelImage2.length<pixelImage1.length)
			pixelCount = pixelImage2.length;
		for(int i = 0 ; i < pixelCount ; i++) 
		{
